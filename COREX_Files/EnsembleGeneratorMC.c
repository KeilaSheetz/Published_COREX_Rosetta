/**** Monte Carlo Ensemble Generator Application ****/
/**** Changed int2bin ****/
...
extern void calc_stat_weight (asa_atom_rec_p atom_list,int num_atoms);
extern float rand1(void);
extern void Init_Code(float P, int Code_Len, char *Code);
...
while(end)
     {
      printf("\nToo large of an ensemble? [%s] > ",Default_Answer); 
      fgets(instring,Str_Len,stdin);
      ZotNewLine(instring);
      if (!strlen(instring))
...
printf("\nEnter Number of States Monte Carlo Sampled Per Partition [%d] > ",Def_Nsamples); 
fgets(instring,Str_Len,stdin);
ZotNewLine(instring);
if (!strlen(instring))
    Nsamples=Def_Nsamples;
else
    sscanf(instring,"%d",&Nsamples);

printf("\nTotal Number of States Sampled = %d * %d partitions = %d\n",Nsamples,window_size,Nsamples*window_size);
time_hours=(Nsamples*window_size)*72.0/1000000.0;
...
while(end)
     {
      printf("\nToo many states sampled? [%s] > ",Default_Answer); 
      fgets(instring,Str_Len,stdin);
      ZotNewLine(instring);
      if (!strlen(instring))
          strcpy(Answer,Default_Answer);
...
idum=-1;
iff=0;
RT_Inverse=(float)(1.0/(1.9872*Current_Temp));
sprintf(out_fname,"%s.MC.%d.%d",inFName,window_size,Min_Res_Win);
printf("\n\nOut file = %s",out_fname);
outfile2=fopen(out_fname,"w");
for(partition=1;partition<=Npartition;partition++)
   {
    printf("\n\nCurrent partition %d of %d total partitions.\n",partition,Npartition);
    printf("                                      ");
    Native_State();
    Code_Len=Nunits[partition];
    state_count=0;
    if (partition == 1)                                           /* Explicitly add the fully unfolded state to the partition function */
       {
        state_count++;
        for(window=0;window<Code_Len;window++) stateflag[window]='1';
        ...
        asa_calculate(atom_list,num_atoms,slice_width,solvent_radius);
        for (i=0;i<num_atoms;i++) Atom_Area[i]=(double)atom_list[i].area;
        calc_stat_weight(atom_list,num_atoms);
        state_number=0;
        for(j=0;j<Nunits[partition];j++)
           {
            if(stateflag[j]=='1') state_number=state_number+pow(2,j);
           }
        fprintf(outfile2,"%d %ld %ld%8.4f%9.2f%9.2f%9.2f",partition,state_count,state_number,Fraction_Folded,Sconf,delASA_ap,delASA_pol);
        fprintf(outfile2,"\n");
        factor=-log(prob)/(dG25*RT_Inverse);
        if (dG25 < 5000.0) factor=-log(prob)/(5000.0*RT_Inverse);
        used_states[state_count]=state_number;
       }
    ntries=1;
    while(ntries<=Nsamples)
       {
        printf("\r");
        printf("Progress: %6d  of %d  States Sampled",ntries,Nsamples);
        fflush(stdout);
        Pfold=rand1();
        Init_Code(Pfold,Code_Len,stateflag);
        state_number=0;
        for(j=0;j<Nunits[partition];j++)
           {
            if(stateflag[j]=='1') state_number=state_number+pow(2,j);
        ...
        if (state_count!=0)
           {
            for(istate=1;istate<=state_count;istate++)
               {
                if (used_states[istate]==state_number) repeat=1;
               }
           }
        num_folded=0;
        num_unfolded=0;
        for(window=0;window<Code_Len;window++)
           {
            if(stateflag[window]=='0')num_folded++;
            if(stateflag[window]=='1')num_unfolded++;
           }
        if (num_folded==Code_Len) repeat=1;
        if (num_unfolded==Code_Len) repeat=1;
        if (repeat==0)
           {
            num_atoms=load_atoms_range(&atom_list);
            if (!asa_assign_radii(atom_list,num_atoms,NULL,0))
            ...
            if (dG<0.0) dG=0.0;
            dG=dG*factor;
            selection=exp(-dG*RT_Inverse);
            selRand=rand1();
            if (selection<selRand) repeat=1;
           }
        if (repeat==0)
           {
            state_count++;
            ...
            for(i=1;i<=Nunits[partition];i++)       
               {
                if (stateflag[i-1]=='0')
                   {
                    for(j=First_Residue[partition][i];j<=Last_Residue[partition][i];j++) fprintf(outfile2,"%8.3f",Fraction_amide_exposed[j]);
                   }  
               }
            fprintf(outfile2,"\n");
           }
...
void calc_stat_weight (asa_atom_rec_p atom_list, int num_atoms)
{
int i,j,k,atom_number,jj;
char junk[7];
float Sum_U_Apolar,Sum_U_Polar;
float ASA_State_Apolar,ASA_State_Polar;
float ASA_side_chain,ASA_Amide;
float dCp,dH60,dH25,dSsolv_ap25,dSsolv_pol25;
float dASA_ap,dASA_pol,SconfN;
...
for(i=0;i<num_atoms;i++)
   { 			                                 
    strncpy(junk,atom_list[i].type,4);
    if (junk[1]=='C')
       {
        ...
        for(j=First_Residue[partition][i];j<=Last_Residue[partition][i];j++)
           {
            ...
            for(k=Begin_Atom_Res[j];k<=Last_Atom_Res[j];k++)
               {
                strncpy(junk,atom_list[atom_number].type, 4); 
...
dASA_ap=ASA_State_Apolar+Sum_U_Apolar-ASA_N_Apolar;
dASA_pol=ASA_State_Polar+Sum_U_Polar-ASA_N_Polar;
delASA_ap=dASA_ap;
delASA_pol=dASA_pol;	
...
dSsolv_ap25=dASA_ap*aCp*log((Current_Temp/TsApolar));
dSsolv_pol25=dASA_pol*bCp*log((Current_Temp/TsPolar));
...
long mbig,mseed,mz;
float ran3,fac;
long i,ii,ij,k,kk,kj,mj,mjj,mk;
...
void Init_Code(float P, int Code_Len, char *Code)
{ 
int i;
